...
(define (parse sexp)
  (cond
    ...
    [(eq? sexp 'number) (numberT)]
    [(eq? sexp 'boolean) (booleanT)]
    [(symbol? sexp) (idS sexp)]
    [(list? sexp)
     (let
         ([parse-binding (λ (b) (binding (first b) (parse-type (third b)) (parse (fourth b))))])
       (case (first sexp)
         ;; types
         ;; operations
         [(+)      (safe-op + sexp)]
         ...
         [(and)      (safe-op (procedure-rename (lambda x (andmap identity x)) 'and) sexp)]
         [(or)      (safe-op (procedure-rename (lambda x (ormap identity x)) 'or) sexp)]
...
(define (check-fun-duplicates sexp)
  (let
      ([duplicates (check-duplicates (second sexp))]
       [parse-params (λ (b) (param (first b) (parse-type (third b))))]
       )
    (if duplicates
        (error (format "parser: parámetro definido dos veces: ~a" duplicates))
        (funS (map parse-params (second sexp)) (parse-type (fourth sexp)) (parse (fifth sexp))))))
...
;; Toma una expresión de tipos y devuelve el tipo correspondiente
;; expr: s-expression -> SCFWBAE
;; TODO: Revisar si existe '->
(define (parse-type expr)
  (if (list? expr)
      (funT (foldr (lambda (x y) (if (not (equal? x '->))
                                     (cons (parse x) y)
                                     y))
                   '() expr))
      (parse expr)
